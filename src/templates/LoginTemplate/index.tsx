...
const LoginTemplate = () => {
  ...
  const handleSignIn = (e: React.FormEvent) => {
    ...
    if (!storage.length) {
      return toast('Cadastro nao encontrado!')
    }
      ...
      {showLogin && (
        <S.Form onSubmit={handleSignIn}>
          ...
          <Input
            ...
            handleOnChange={(value: string) => setLoginData({ ...loginData, email: value })}
            type="email"
          />
          <Input
            required
            ...
            handleOnChange={(value: string) => setLoginData({ ...loginData, password: value })}
          />
          <p>
            Ainda não tem cadastro? <a onClick={() => setShowLogin(false)}>Cadastre-se aqui!</a>
          </p>

          <Button type="submit" disabled={!(loginData?.email && loginData?.password)}>
            Entrar
          </Button>
        </S.Form>
      )}
      ...
      {!showLogin && (
        <S.Form onSubmit={handleSignUp}>
          ...
          <Input
            required
            label="Nome"
            handleOnChange={(value: string) => setFieldValues({ ...fieldValues, name: value })}
          />
          <Input
            required
            label="E-mail"
            type="email"
            handleOnChange={(value: string) => setFieldValues({ ...fieldValues, email: value })}
          />
          <Input
            required
            label="Telefone"
            type="tel"
            mask="phone"
            handleOnChange={(value: string) => setFieldValues({ ...fieldValues, phone: value })}
          />
          <Input
            required
            label="Senha"
            type="password"
            handleOnChange={(value: string) => setFieldValues({ ...fieldValues, password: value })}
          />
          <p>
            já tem cadastro? <a onClick={() => setShowLogin(true)}>Clique aqui!</a>
          </p>
